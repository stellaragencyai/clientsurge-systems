 ...
 * Access rule: authenticated user or signed setup token required before showing order data.
 */
...
export default function SetupStatus() {
  ...
  const setupToken = searchParams.get("token") || searchParams.get("setup_token") || "";
  const hasAccess = isAuthenticated || setupToken.length >= 16;
  ...
  const fetchStatus = async () => {
    try {
      const payload = { order_id: orderId, setup_token: setupToken || undefined };
      const [orderRes, progressRes] = await Promise.all([
        base44.functions.invoke("getOrderStatus", payload),
        base44.functions.invoke("getActivationProgress", payload),
      ]);
  ...
  useEffect(() => {
    if (isLoadingAuth || isLoadingPublicSettings || !hasAccess) return;
    if (!orderId) {
      setLoading(false);
      return;
    }
  ...
  }, [orderId, setupToken, isLoadingAuth, isLoadingPublicSettings, hasAccess]);
  ...
  if (isLoadingAuth || isLoadingPublicSettings || loading) {
    return (
    ...
    <div style={{ minHeight: "100vh", background: "#0A0F1E", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 20px" }}>
      <div style={{ maxWidth: 520, width: "100%" }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          <div style={{ fontSize: 48, marginBottom: 12 }}>Setup</div>
        ...
        {hasActivationError && (
          <div style={{ marginBottom: 24, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(248,113,113,0.25)", borderRadius: 16, padding: 18 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 14 }}>
              <button onClick={() => setShowSupportOptions((value) => !value)} style={{ borderRadius: 999, border: "none", background: "linear-gradient(135deg,#0088CC,#003B8F)", color: "#fff", fontSize: 12, fontWeight: 700, padding: "10px 14px", cursor: "pointer" }}>
                Contact Support
              </button>
              <a href="mailto:support@clientsurgesystems.com?subject=Setup%20status%20help" style={{ borderRadius: 999, border: "1px solid rgba(255,255,255,0.15)", color: "#fff", fontSize: 12, fontWeight: 600, padding: "10px 14px", textDecoration: "none" }}>
                Email support
              </a>
            </div>
            {showSupportOptions && (
            ...
            return (
              <div key={stage.key} style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: isDone ? "linear-gradient(135deg,#00D4FF,#00FFB3)" : isActive ? "rgba(0,212,255,0.15)" : "rgba(255,255,255,0.06)", border: isActive ? "2px solid #00D4FF" : isDone ? "none" : "1px solid rgba(255,255,255,0.1)", fontSize: 13, fontWeight: 800, color: isDone ? "#0A0F1E" : isActive ? "#00D4FF" : "rgba(255,255,255,0.3)" }}>
                    {isDone ? "✓" : index + 1}
                  </div>
