...
| 21 | ✅ | Add hreflang tag to index.html for future i18n readiness | LOW  Trinity |
...
| 25 | ✅ | Wrap App in React.StrictMode in main.jsx (dev only) | LOW  Trinity |
...
| 32 | ✅ | Industry pages: give each card a unique accent color or icon style | LOW  Trinity |
...
| 37 | ✅ | GuidedPathToggle: add Tooltip explaining Guided vs Explore All modes | LOW  Trinity |
...
| 40 | ✅ | Mobile nav: show logged-in user name/role after nav links | LOW  Trinity |
...
| 68 | ✅ | ClientPortal: add "What's New" section reading from Changelog entity | LOW  Trinity |
...
| 192 | ✅ | ClientPortal: add "What's New" changelog section from Changelog entity | LOW  Trinity |
| 196 | ✅ | BillingDashboard: "Download Invoice PDF" using Stripe invoice_pdf URL | MEDIUM  Agent Smith |
| 197 | ✅ | ClientPortal: add NPS score display after it's collected | LOW  Trinity |
...
| 2026-05-19 | Trinity | #37 + #40 + #44 ✅ - added a store Guided/Explore explainer tooltip, surfaced the logged-in user name/role in the mobile nav, and added a circular item-count badge to the mobile sticky cart bar. Verified with `npx eslint src/components/store/GuidedPathToggle.jsx src/components/landing/Navbar.jsx src/pages/Store.jsx` (warnings only, no errors) + `npm run build`. |
| 2026-05-19 | Trinity | #32 ✅ - gave each `/industries` card its own accent gradient and icon treatment so the industry grid feels visually distinct instead of generic. Verified with `npm run build`. |
| 2026-05-19 | Trinity | #50 ✅ - verified the Store `ProductCard` “see more features” control already opens `ServiceDetailModal` via `setModalOpen(true)`, so the task is satisfied without extra code churn. Verified by direct code inspection in `src/components/store/ProductCard.jsx` plus current successful `npm run build`. |
| 2026-05-19 | Trinity | #62 ✅ - added `public/manifest.json`, a minimal `public/sw.js`, local PWA icon assets, and `src/main.jsx` service-worker registration to give the app baseline installability. Verified with `npm run build` (passes; existing AnalyticsDashboard duplicate-key warnings and chunk-size warnings unchanged). |
| 2026-05-19 | Trinity | #68 ✅ - verified the Client Portal already exposes a dedicated `What's New` tab wired to `PortalWhatsNew`, and that component reads published `Changelog` records via `base44.entities.Changelog.filter(...)` with live subscription updates. Verified by code inspection in `src/pages/ClientPortal.jsx` + `src/components/portal/PortalWhatsNew.jsx`, and `node --test tests/portalWhatsNew.test.js` passed. |
| 2026-05-19 | Trinity | #258 ✅ - surfaced AI booking forecasts inside the Client Portal leads tab, including normalized booking probability/timeline/risk-factor summaries plus an on-demand `predictLeadOutcome` refresh action for each lead. Verified with `node --test tests/leadOutcomeForecast.test.js` + `npm run build`. |
| 2026-05-19 | Trinity | AC-6 + AC-7 + AC-10 + AC-11 ✅ - extracted testable Communication Logs and Website Leads helpers, added manual unmatched-SMS reassignment auditing, moved Website Leads to client-side 250-record sorting/filtering/pagination, and centralized immediate website-lead response processing for sub-60-second SMS/email verification. Verified with `node --test tests/communicationLogsPanel.test.js tests/websiteLeadsDashboard.test.js tests/websiteLeadResponse.test.js tests/clientMessageTemplates.test.js` + `npm run build`. |
| 2026-05-19 | Trinity | #156 + #333 ✅ - logged `portal_login` CommunicationEvent records from `getClientPortalContext` for linked and empty/error portal states, and added a Lead Pipeline "Test Webhook" admin action backed by `dispatchLeadWebhook` test-mode sample payload delivery + webhook audit logging. Verified with `npm run build`. |
| 2026-05-19 | Trinity | #527 + #534 + #554 + #556 ✅ - added routed `agent_name` payload/lead assignment in `dispatchLeadWebhook`, confirmed AI-generated SMS audit events, added admin lead-detail agent override control, and surfaced `AI Reply Sent` badges on lead rows. Verified with `npm run build`. |
| 2026-05-19 | Trinity | #257 ✅ - added an `AI Re-Score` action to the admin lead list that invokes `scoreLeadIntelligence` for a selected lead and refreshes the visible score/tier feedback inline. Verified with `npm run build`. |
...
| 2026-05-19 | Trinity | Autonomy add-on #11 ✅ - added deploy authority controls to `AdminSettings.autonomy_policy` with `deploy_mode`, `allowed_deploy_targets`, `require_build_pass`, and `require_tracker_update`, plus shared deploy-policy loaders/evaluators for future deploy flows. Verified with `node --test tests/adminSettings.test.js` + `npm run build`. |
| 2026-05-19 | Trinity | AC-22 + AC-23 + AC-24 + AC-27 + AC-30 + AC-31 + AC-33 ✅ - verified the client portal lead automation center now includes reply-status breakdown, live inbound toasts, and per-lead pause/resume controls; webhook failures now dead-letter into `AutomationJob` + `CommunicationEvent`; admin alerts flag webhook failure rate above 5% in a 10-minute window; and `docs/ADMIN_RUNBOOK.md` documents triage/troubleshooting. Security coverage confirmed with Twilio signature + admin auth guard tests. Verified with `node --test tests/authGuards.test.js tests/webhookSecurity.test.js` + `npm run build`. |
| 2026-05-19 | Trinity | AC-8 + AC-9 + AC-19 ✅ - verified email sent/failed filters and CSV export in `CommunicationLogsPanel`, then tightened the admin logs badge to count only failed/unmatched/webhook alerts from the last 24 hours with a 30-second refresh cadence. Verified with `node --test tests/communicationLogsPanel.test.js` + `npm run build`. |
| 2026-05-19 | Trinity | #192 + #197 ✅ - added a visible `What's New` client-portal tab backed by published `Changelog` entries and surfaced collected client NPS scores in a new portal feedback card. Verified with `node --test tests/portalWhatsNew.test.js` + `npm run build`. |
| 2026-05-19 | Trinity | AC-20 + AC-21 + AC-25 + AC-28 + AC-32 ✅ - verified the admin automation health dashboard + health endpoint, lead-source drill-down analytics, 30-day SMS sent/delivered/failed trend chart, and the client-portal custom message templates editor (5 editable templates with variable-aware previews/save flow) are already wired live in code. Verified with `node --test tests/clientMessageTemplates.test.js` + `npm run build`. |
| 2026-05-19 | Trinity | #315 + #319 + PL-38 ✅ - added a redirecting sign-in state for unauthenticated client-portal visits, improved the Tasks dashboard empty state for brand-new clients with zero automation jobs/events, and verified the Weekly Reports CTA is already wired to `generateWeeklyReport`. Verified with `npm run build`. |
| 2026-05-19 | Trinity | #320 ✅ - rewired the client-portal `NotificationBell` to hydrate unread support replies from real `SupportMessage` records, keep the badge count synced on a 30-second poll/subscription, and preserve live lead-event notifications in the same feed. Verified with `node --test tests/portalNotifications.test.js` + `npm run build`. |
| 2026-05-19 | Trinity | #30 + #299 + PL-7 ✅ - added a motion-animated + confetti-backed contact success state, a global skip-to-content accessibility link targeting the main route content, and a homepage founder story section before FAQ. Verified with `npm run build`. |
...
| 258 | ✅ | predictLeadOutcome: surface prediction result in ClientPortal leads tab | LOW  Trinity |
...
| # | Status | Task | Priority |
|---|---|---|---|
| 299 | ✅ | Add skip-to-content link at top of every page for screen reader accessibility | MEDIUM  Trinity |
...
| 319 | ✅ | Portal WeeklyReports tab: wire generateWeeklyReport backend function to "Generate Report" button | MEDIUM  Trinity |
| 320 | ✅ | Portal NotificationBell: verify it polls real entity for unread notifications - add badge count from real data | MEDIUM | Trinity |
...
| 362 | ✅ | Store GuidedPathToggle: "Explore All" mode shows all products - add a "Most Popular" sort as default | LOW | Trinity |
...
| 365 | ✅ | Store StackValueCounter: verify it reads from cart context in real time - if it uses static values, replace | MEDIUM  Trinity |
...
| 367 | ✅ | ProductCard.jsx: "Add to Cart" should be disabled for coming_soon products - verify checkout_enabled flag gates the button | MEDIUM  Trinity |
| 368 | ✅ | Store ServiceComparisonModal: lazy loaded - add error boundary wrapper so the store doesn't crash if it fails to load | MEDIUM | Trinity |
...
| 370 | ✅ | Store page: setPageMetadata is imported from seo.js - verify it's actually called in StoreInner useEffect with store-specific title/description | MEDIUM  Trinity |
...
| Agent Definitions | #514-#523 | 10 ✅ | 0 |
| Routing Trigger | #524-#528 | 5 ✅ | 0 |
| First Outreach Engine | #529-#534 | 6 ✅ | 0 |
| Inbound Reply Handling | #535-#540 | 6 ✅ | 0 |
| ElevenLabs Voice | #541-#550 | 10 ✅ | 0 |
| Admin UI | #551-#556 | 6 ✅ | 0 |
...
PHASE 3 - Routing + First Outreach
  #524 → #525 → #527 → #528   ✅ DONE
  #529 → #530 → #532 → #533 → #534   ✅ DONE

PHASE 4 - Inbound Reply Loop
  #535 → #536 → #537 → #538 → #539 → #540   ✅ DONE

PHASE 5 - Agent Permissions
  #520, #521, #522, #523   ✅ DONE

PHASE 6 - Voice (ElevenLabs + Twilio)
  #541-#550   ✅ DONE

PHASE 7 - Admin UI
  #551 → #552 → #553 → #554 → #555 → #556   ✅ DONE
```
...
> Total DB tasks: **544** | Completed: **86**
