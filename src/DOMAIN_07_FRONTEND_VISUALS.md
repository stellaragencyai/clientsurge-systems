...
|---|---|---|---|---|---|---|
| 76 | ⏳ | Verify Stripe publishable key is ONLY in frontend (not sk_live_ anywhere) | A | — | — | ~20 min |
...
| 53 | ⏳ | Audit all form inputs for iOS zoom issue (font-size < 16px) | A | — | — | ~45 min |
...
| 243 | ⏳ | Final: test all CTA buttons across mobile (375px, 390px, 414px) | A | — | → ALL sign-off | ~1 hr |
...
| 15 | ⏳ 🟢 | DemoBookingModal time slot grid: force 2-col on viewports < 480px | A | — | — | ~20 min |
| 26 | ⏳ | Add dark mode ☀️/🌙 toggle to Navbar desktop + mobile menu | A | #27 | — | ~30 min |
| 27 | 🔄 | Implement ThemeProvider from next-themes so dark mode class is applied | A | — | → A (#26 toggle) | ~45 min |
| 28 | 🔄 | Standardize primary CTAs to blue gradient; gold = store/checkout only | A | — | — | ~1 hr |
| 31 | ⏳ | pages/Industries: add gradient hero section with industry grid icons | A | — | — | ~1 hr |
| 33 | ⏳ | Mobile sticky cart bar: add padding-top: 72px to main content when visible | A | — | — | ~20 min |
...
| 42 | ⏳ | Store ProductCard on mobile (375px): reduce "Add to Cart" font to 10px | A | — | — | ~10 min |
| 44 | ⏳ | Mobile sticky cart bar: add circular badge with items.length count | A | — | — | ~15 min |
| 45 | ⏳ | Store page: add "Talk to a Human" escape valve CTA below product grid | A | — | — | ~20 min |
| 49 | ⏳ | Store: Guided mode no industry → show all non-coming-soon products | A | — | — | ~30 min |
| 52 | ⏳ | MobileCallBar: pull phone number from AdminSettings instead of hardcoding | A | — | — | ~30 min |
| 54 | ⏳ | DemoBookingModal step 2: set min-height:48px on date/time inputs | A | — | — | ~10 min |
...
| 16 | ⏳ 🟢 | CookieConsent banner: add bottom: 80px on mobile (avoid MobileCallBar overlap) | A | — | — | ~10 min |
| 17 | ⏳ 🟢 | FAQ accordion: add border-bottom tap target on mobile | A | — | — | ~10 min |
...
| 40 | ⏳ | Mobile nav: show logged-in user name/role after nav links | A | — | — | ~20 min |
