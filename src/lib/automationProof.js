...
export function buildAutomationProof(events = []) {
  ...
  return AUTOMATION_DEFINITIONS.map((def) => {
    ...
    const sent = matching.filter(isSent);
    const failed = matching.filter(isFailure);
    const skipped = matching.filter(isSkippedOrSuppressed);
    const latest = matching[0] || null;

    let status = 'no_signal';
    if (failed.length > 0 && sent.length === 0) status = 'failing';
    else if (sent.length > 0 && failed.length === 0) status = 'proven';
    else if (sent.length > 0 && failed.length > 0) status = 'mixed';
    else if (skipped.length > 0) status = 'guarded';
