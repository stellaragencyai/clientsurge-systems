    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-200 flex flex-col">
      ...
      <div className="h-40 bg-gradient-to-br from-primary/10 to-primary/5 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <span className="bg-white/90 text-xs font-bold text-primary px-2.5 py-1 rounded-full shadow-sm">
            {resource.category}
          </span>
        </div>
        {resource.is_gated && (
          <div className="absolute top-3 right-3">
            <span className="bg-foreground/80 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
              FREE
            </span>
          </div>
        )}
      ...
      <div className="p-5 flex flex-col flex-1">
        <p className="text-[11px] font-bold uppercase tracking-widest text-primary/70 mb-1.5">
...
export default function Library() {
  ...
  useEffect(() => {
    const cleanup = setPageMetadata({
      title: "Free Resources & Automation Guides | ClientSurge Systems",
      description: "Download free automation guides, templates, checklists, and playbooks for local service businesses.",
  ...
  useEffect(() => {
    base44.entities.Resource.filter({ is_published: true }, "-created_date", 50)
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-32 pb-16 px-6 text-center bg-gradient-to-b from-primary/5 to-background">
        ...
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed">
          Free guides, templates, checklists, and playbooks to help local service businesses capture more leads and automate follow-up.
        </p>
        ...
        <div className="mt-8 max-w-md mx-auto relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 rounded-xl border border-border bg-white text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 shadow-sm"
          />
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="text-sm border border-border rounded-lg px-3 py-2 bg-background focus:outline-none focus:ring-2 focus:ring-primary/40"
            >
              {INDUSTRIES.map(ind => (
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filtered.map(resource => (
              <ResourceCard key={resource.id} resource={resource} onDownload={setSelectedResource} />
            ))}
          </div>
        )}
      </div>
