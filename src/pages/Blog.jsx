  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0A0F1E", padding: "60px 20px" }}>
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        ...
        <p style={{ color: "rgba(255,255,255,0.72)", fontSize: 14, margin: "0 0 18px" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, margin: "0 0 36px" }} aria-label="Blog topics">
          {["Missed-call recovery", "AI lead follow-up", "AI voice agents", "Booking automation", "Conversion systems"].map((topic) => (
            <span key={topic} style={{ color: "rgba(255,255,255,0.78)", border: "1px solid rgba(255,255,255,0.18)", borderRadius: 9999, padding: "4px 10px", fontSize: 11 }}>
              {topic}
            </span>
          ))}
        </div>
          ...
          {POSTS.map(p => (
            <div key={p.slug} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "22px 24px" }}>
              <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                ...
                <span style={{ color: "rgba(255,255,255,0.68)", fontSize: 11, alignSelf: "center" }}>{p.date}</span>
              </div>
              <h2 style={{ color: "#fff", fontSize: 17, fontWeight: 800, margin: "0 0 8px", lineHeight: 1.3 }}>{p.title}</h2>
              <p style={{ color: "rgba(255,255,255,0.76)", fontSize: 13, lineHeight: 1.7, margin: "0 0 14px" }}>{p.excerpt}</p>
              <span style={{ color: "#00D4FF", fontSize: 12, fontWeight: 600 }}>Read the field note</span>
            </div>
          ))}
        </div>
      </div>
