...
const PACKAGE_SUMMARIES = {
  starter_system: {
    ...
    shortDescription: "Instant lead response and missed-call text-back.",
  },
  growth_system: {
    problem: "We need follow-up and booking handoff handled.",
    shortDescription: "Starter plus 14-day nurture and AI booking handoff.",
    recommended: true,
  },
  pro_system: {
    problem: "We want the complete six-automation system.",
    shortDescription: "Growth plus old lead reactivation and review requests.",
  },
};
...
async function invokePublicCheckoutSession(payload) {
  ...
  if (!response.ok || !checkoutUrl) {
    const message = result?.error || result?.data?.error || `Checkout server returned HTTP ${response.status || "error"}.`;
  ...
  return { data: { ...result, url: checkoutUrl }, url: checkoutUrl };
}
...
export default function ProductSignup() {
  ...
  const validate = () => {
    ...
    if (!industry.trim()) return "Please select your industry.";
  ...
  const handleCheckout = useCallback(async (event) => {
    ...
    try {
      ...
      if (!url) throw new Error(response?.data?.error || response?.error || "No checkout URL returned by the server.");

      setSuccess(true);
      window.setTimeout(() => { window.location.assign(url); }, 500);
    } catch (err) {
      const msg = err?.message || "Checkout could not be started.";
      const suffix = err?.request_id ? ` Request ID: ${err.request_id}.` : "";
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col" style={{ fontFamily: "Montserrat, system-ui, sans-serif" }}>
      <div data-route-verify="product-signup" data-selected-package={selectedPackage} className="absolute -left-[9999px] -top-[9999px]" aria-hidden="true">
      ...
      <header className="border-b border-gray-100 bg-white">
        <div className="max-w-4xl mx-auto px-4 py-5 flex items-center justify-between gap-4">
          <Link to="/" className="flex items-center gap-2 text-gray-900 no-underline" aria-label="ClientSurge Systems home">
            <svg className="w-7 h-7" viewBox="0 0 24 24" fill="#005691" aria-hidden="true"><path d="M12 2L4 6v6c0 5 3.5 9.5 8 10 4.5-.5 8-5 8-10V6l-8-4z" /></svg>
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 py-8 md:py-12">
        <div className="text-center mb-8">
          <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: "#0088CC" }}>Secure Checkout</p>
          <h1 className="text-2xl md:text-3xl font-black text-gray-900 mb-2">Complete your ClientSurge signup</h1>
          <p className="text-sm text-gray-500">Selected package: <strong className="text-gray-900">{currentPkg.name}</strong></p>
        </div>
            ...
            {CHECKOUT_PACKAGES.map((pkg) => {
              ...
              return (
                <button
                  ...
                  className={`relative text-left p-5 rounded-xl border-2 transition-all ${isSelected ? "border-blue-500 bg-blue-50 shadow-md" : "border-gray-200 bg-white hover:border-blue-300"}`}
        ...
        <form onSubmit={handleCheckout} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 md:p-8">
          ...
          <p className="text-sm text-gray-500 mb-6">Enter your details to continue to the secure Stripe payment page.</p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label htmlFor="fullName" className="block text-sm font-semibold text-gray-700 mb-1.5">Full Name *</label>
              <input id="fullName" name="name" type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="John Smith" autoComplete="name" required className="w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" style={{ fontSize: "16px" }} />
            </div>
            <div className="md:col-span-2">
              <label htmlFor="businessName" className="block text-sm font-semibold text-gray-700 mb-1.5">Business Name *</label>
              <input id="businessName" name="organization" type="text" value={businessName} onChange={(e) => setBusinessName(e.target.value)} placeholder="Acme Landscaping LLC" autoComplete="organization" required className="w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" style={{ fontSize: "16px" }} />
            </div>
            <div>
              <label htmlFor="email" className="block text-sm font-semibold text-gray-700 mb-1.5">Email Address *</label>
              <input id="email" name="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="john@yourbusiness.com" autoComplete="email" required className="w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" style={{ fontSize: "16px" }} />
            </div>
            <div>
              <label htmlFor="phone" className="block text-sm font-semibold text-gray-700 mb-1.5">Phone Number *</label>
              <input id="phone" name="phone" type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="(555) 123-4567" autoComplete="tel" required className="w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" style={{ fontSize: "16px" }} />
            </div>
            <div className="md:col-span-2">
              <label htmlFor="industry" className="block text-sm font-semibold text-gray-700 mb-1.5">Industry / Business Type *</label>
              <select id="industry" name="industry" value={industry} onChange={(e) => setIndustry(e.target.value)} required className="w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white" style={{ fontSize: "16px" }}>
          ...
          <label className="mt-5 flex items-start gap-3 rounded-xl border border-blue-100 bg-blue-50 p-4 text-sm text-gray-700">
            ...
            <span>I agree that ClientSurge Systems may contact me by email, phone, or SMS about this purchase and setup. Message/data rates may apply. Reply STOP to opt out. See <Link to="/privacy" className="font-semibold underline">Privacy</Link>, <Link to="/terms" className="font-semibold underline">Terms</Link>, and <Link to="/sms-terms" className="font-semibold underline">SMS Terms</Link>.</span>
          </label>

          {error && <div className="mt-6 rounded-xl border border-red-200 bg-red-50 p-4" role="alert"><p className="text-sm font-semibold text-red-900">Checkout Error</p><p className="text-sm text-red-700 mt-1">{error}</p></div>}
          {success && <div className="mt-6 rounded-xl border border-green-200 bg-green-50 p-4 text-center" role="status"><p className="text-sm font-bold text-green-900">Redirecting to secure checkout…</p><p className="text-xs text-green-700 mt-1">You will be transferred to Stripe to complete your payment safely.</p></div>}
          ...
          {!success && (
            <button type="submit" disabled={loading} className="mt-6 w-full py-4 rounded-xl font-black text-white text-lg flex items-center justify-center gap-2 transition-all disabled:opacity-60" style={{ background: loading ? "#9cb3c9" : "linear-gradient(135deg, #003B8F 0%, #0088CC 60%, #00AEEF 100%)", boxShadow: loading ? "none" : "0 4px 16px rgba(0,136,204,0.35)", minHeight: "56px" }}>
              {loading ? "Preparing secure checkout…" : `Continue to Secure Checkout — ${currentPkg.name}`}
            </button>
          )}

          {!success && <p className="mt-4 text-center text-xs text-gray-400">Stripe checkout · encrypted payment form · no card details stored by ClientSurge</p>}
        </form>

        <div className="mt-6 text-center"><p className="text-xs text-gray-400">Need help? <Link to="/contact" className="font-semibold underline" style={{ color: "#0088CC" }}>Contact support</Link> · <Link to="/pricing" className="font-semibold underline" style={{ color: "#0088CC" }}>Compare packages</Link></p></div>
      </main>

      <footer className="border-t border-gray-100 bg-white py-6">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <p className="text-xs text-gray-400">© {new Date().getFullYear()} ClientSurge Systems · AI-powered sales systems for local service businesses</p>
