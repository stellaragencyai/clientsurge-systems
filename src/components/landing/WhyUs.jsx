...
function AnimatedStat({ value, label, delay }) {
  ...
  return (
    ...
    >
      <div className="font-display text-3xl md:text-4xl font-bold mb-1 text-primary">{value}</div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-[1fr_auto_1fr] gap-0 items-stretch max-w-4xl mx-auto mb-14">
          ...
          <div className="hidden md:flex items-center justify-center px-4">
            <div className="w-9 h-9 rounded-lg flex items-center justify-center shadow-md flex-shrink-0" style={{ background: "linear-gradient(135deg,#003B8F,#00AEEF)", border: "2px solid white" }}>
              <ArrowRight className="w-4 h-4 text-white" />
            </div>
          </div>

          {/* Right - Our System */}
          <div className="rounded-lg p-7 flex flex-col gap-4" style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.58) 0%, rgba(238,248,255,0.42) 100%)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", border: "1.5px solid rgba(0,174,239,0.26)", boxShadow: "0 4px 20px rgba(0,88,160,0.07), inset 0 1px 0 rgba(255,255,255,0.7)" }}>
            <div className="flex items-center gap-2 mb-1">
              <span className="w-2 h-2 rounded-full bg-primary" />
              <p className="text-xs font-black uppercase tracking-widest text-primary">ClientSurge Systems</p>
            </div>
            {systemItems.map((t, i) => (
              <div key={i} className="flex items-start gap-3">
                <div className="w-5 h-5 rounded-lg flex-shrink-0 flex items-center justify-center mt-0.5 bg-primary/15">
        ...
        {/* ── Animated Stats Bar ── */}
        <div className="rounded-lg mb-14 py-8 px-6 grid grid-cols-2 md:grid-cols-4 gap-8 bg-primary/5 border border-primary/20">
        ...
        <div ref={sectionRef} className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-12">
          {reasons.map((r, i) => {
            ...
            return (
              <div
                key={i}
                className="group flex flex-col gap-3 p-5 rounded-lg transition-all duration-300"
                style={{
                  background: "linear-gradient(135deg, rgba(255,255,255,0.64) 0%, rgba(238,248,255,0.42) 100%)",
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(0,174,239,0.38)"; e.currentTarget.style.boxShadow = "0 12px 32px rgba(0,88,160,0.14), inset 0 1px 0 rgba(255,255,255,0.8)"; e.currentTarget.style.transform = "translateY(-4px)"; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(0,174,239,0.15)"; e.currentTarget.style.boxShadow = "0 3px 16px rgba(0,0,0,0.05), inset 0 1px 0 rgba(255,255,255,0.7)"; e.currentTarget.style.transform = "translateY(0)"; }}
              >
                <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-300 group-hover:scale-110 bg-primary/10 border border-primary/25">
        ...
        {/* ── Timeline Strip ── */}
        <div className="rounded-lg overflow-hidden mb-12 border border-primary/20">
          <div className="grid grid-cols-2 md:grid-cols-4">
            {["Day 1: Kickoff Call", "Days 2-4: Build & Integrate", "Days 5-7: Test & Launch", "Day 30+: Ongoing Optimization"].map((step, i) => (
              <div key={i} className={`px-5 py-5 text-center relative ${i < 3 ? "border-r border-primary/10" : ""}`} style={{ background: "rgba(0,174,239,0.04)" }}>
                {i < 3 && (
                ...
                <div className="w-7 h-7 rounded-lg flex items-center justify-center mx-auto mb-2 text-xs font-black text-white" style={{ background: "linear-gradient(135deg,#003B8F,#00AEEF)" }}>{i + 1}</div>
                <p className="text-xs font-semibold text-foreground/80 leading-snug">{step}</p>
              </div>
            ))}
          </div>
          <div className="px-6 py-4 text-center" style={{ background: "linear-gradient(135deg,#003B8F 0%,#0088CC 60%,#00AEEF 100%)" }}>
            <p className="text-xs font-bold uppercase tracking-widest text-white/60 mb-0.5">After That</p>
            <p className="text-sm font-bold text-white">Everything Runs on Autopilot</p>
          </div>
        </div>
        ...
        <div className="text-center">
          ...
          <button
            ...
            style={{ display: "inline-block", borderRadius: "8px", padding: "2px", background: "linear-gradient(135deg,#003B8F 0%,#0088CC 55%,#00AEEF 100%)", boxShadow: "0 12px 28px rgba(0,88,160,0.22)", border: "none", cursor: "pointer", transition: "box-shadow 0.4s ease" }}
            onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 16px 40px rgba(0,174,239,0.34), 0 6px 18px rgba(0,59,143,0.18)"; }}
