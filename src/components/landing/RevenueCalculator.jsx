    ...
    <section className="py-24 md:py-32 px-6 bg-gradient-to-b from-card to-background">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          <p className="text-xs font-semibold text-primary tracking-widest uppercase mb-4">Revenue Leak Calculator</p>
          <h2 className="font-display text-3xl md:text-4xl lg:text-5xl font-semibold tracking-tight text-foreground">
            How Much Are You <span className="text-red-500">Leaking</span> Every Month?
          </h2>
          <p className="mt-4 text-muted-foreground text-lg">Enter your numbers. See your loss in real time.</p>
        </div>

        <div className="rounded-3xl border border-border overflow-hidden shadow-xl backdrop-blur-sm" style={{ background: 'rgba(255,255,255,0.65)' }}>
          <div className="grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-border">
            ...
            <div className="p-8 md:p-10 space-y-8">
              ...
              <div>
                ...
                <input type="range" min={100} max={5000} step={50} value={ticket}
                  onChange={e => setTicket(+e.target.value)}
                  className="w-full accent-amber-600 cursor-pointer" />
                <div className="flex justify-between text-xs text-muted-foreground mt-1"><span>$100</span><span>$5,000</span></div>
              </div>
              ...
              <div>
                ...
                <input type="range" min={5} max={40} step={1} value={convRate}
            ...
            {/* Results */}
            <div className="p-8 md:p-10 flex flex-col justify-center" style={{ background: 'linear-gradient(135deg, rgba(161,120,35,0.06) 0%, rgba(255,255,255,0.4) 100%)' }}>
              <div className="space-y-5 mb-8">
                <div className="flex items-center justify-between p-4 rounded-xl bg-red-50 border border-red-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{currentBookings} bookings vs {potentialBookings} possible</p>
                    </div>
                  </div>
                ...
                <div className="p-4 rounded-xl border border-primary/20 bg-primary/5">
                  <p className="text-xs text-primary font-semibold mb-1">With ApexFlow (avg. 61% conversion)</p>
                  <p className="text-2xl font-bold text-primary tabular-nums">
                    +${potentialRevenue.toLocaleString()}<span className="text-sm font-medium">/mo</span>
                  </p>
                </div>
              </div>
              ...
              <p className="text-xs text-muted-foreground text-center mt-3">Based on average client results. Individual results vary.</p>
            </div>
          </div>
        </div>
      </div>
