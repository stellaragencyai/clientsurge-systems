...
export default function FAQ() {
  return (
    <section id="faq" className="py-24 md:py-32 px-6 bg-background transition-all duration-700">
        ...
        <Accordion type="single" collapsible className="space-y-3">
          {faqs.map((faq, i) => (
            <AccordionItem
              key={i}
              value={`faq-${i}`}
              className="border border-slate-600/0 hover:border-slate-600 rounded-xl px-6 data-[state=open]:border-slate-600 transition-colors bg-background"
            >
