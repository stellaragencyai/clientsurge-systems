...
const SECTION_TARGETS = {
  business: {
    ...
    description: "Complete or update the core information used to configure your account.",
  },
  "business-hours": {
    step: 1,
    fieldId: "setup-field-business-hours",
    title: "Business hours",
    description: "Confirm when your team is available so routing and after-hours behavior are configured correctly.",
  },
  booking: {
    step: 3,
    fieldId: "setup-field-booking-link",
    title: "Booking link",
    description: "Add or confirm the scheduling link used by your booking automation.",
  },
  messaging: {
    step: 3,
    title: "Messaging setup",
    description: "Review your messaging phone, notification email, and booking configuration.",
  },
  integrations: {
    step: 4,
    title: "Integrations",
    description: "Review the CRM, review link, and lead sources connected to your setup.",
  },
  review: {
    step: 6,
    title: "Final review",
    description: "Review the saved information before submitting it to the installation team.",
  },
};
...
function parseSaveError(err) {
  ...
  return {
    message: validationErrors.length
      ? validationErrors.map((item) => item.message || item.field).filter(Boolean).join(" ")
      : data.error || data.message || err?.message || "Failed to save. Please try again.",
    fieldErrors,
    requestId: data.request_id || err?.request_id || "",
  };
}

function clampStep(value, minimum = 0) {
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return minimum;
  return Math.max(minimum, Math.min(STEPS.length - 1, parsed));
}

function Field({ id, label, required, hint, error, highlight = false, children }) {
  return (
    <div
      id={id}
      className={`space-y-1.5 rounded-xl transition-all ${highlight ? "border border-sky-200 bg-sky-50 p-3 ring-2 ring-sky-100" : ""}`}
...
function BusinessStep({ data, onChange, errors, targetField }) {
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="Business Phone" required error={errors.business_phone}>
          <TextInput
            ...
            onBlur={() => onChange("business_phone", normalizePhone(data.business_phone))}
      ...
      <Field
        id="setup-field-business-hours"
        label="Business Hours"
        hint="When are you open?"
        highlight={targetField === "setup-field-business-hours"}
      >
  ...
  return (
    <div className="space-y-6">
      <Field label="Logo Upload">
        <label className="flex flex-col items-center justify-center w-32 h-32 rounded-xl border-2 border-dashed border-border bg-muted/30 cursor-pointer hover:bg-muted/50 transition-colors">
          {logoUploading ? (
...
function MessagingStep({ data, onChange, errors, targetField }) {
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Twilio Business Phone" hint="Leave blank if you don't have one yet; we will use the business phone for setup checks." error={errors.twilio_business_phone}>
          <TextInput
            value={data.twilio_business_phone}
            onChange={(v) => onChange("twilio_business_phone", v)}
            onBlur={() => onChange("twilio_business_phone", normalizePhone(data.twilio_business_phone))}
        ...
        <Field
          id="setup-field-booking-link"
          label="Booking / Scheduling Link"
          required
          error={errors.booking_link}
          highlight={targetField === "setup-field-booking-link"}
        >
          <TextInput
      ...
      <Field label="Lead Notification Email" required error={errors.lead_notification_email}>
...
function IntegrationsStep({ data, onChange, errors }) {
  return (
    <div className="space-y-5">
      ...
      <Field label="Google Review Link" error={errors.google_review_link}>
        <TextInput
  ...
  const sections = [
    ...
    },
    {
      ...
      fields: [
        ...
        { label: "After-Hours Behavior", value: data.after_hours_behavior },
      ],
    },
    {
      title: "Integrations",
        ...
        <div key={section.title} className="rounded-xl border border-border bg-white overflow-hidden">
          ...
          <div className="divide-y divide-border">
            {section.fields.filter((field) => field.value).map((field) => (
              <div key={field.label} className="flex items-start justify-between px-4 py-2.5 gap-4">
                <p className="text-xs font-semibold text-muted-foreground w-32 flex-shrink-0">{field.label}</p>
                <p className="text-sm text-foreground text-right break-all">{field.value}</p>
              </div>
            ))}
          </div>
        </div>
...
function ErrorPanel({ error, fieldErrors, requestId }) {
  ...
  return (
    <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">
      ...
      {requestId && <p className="mt-2 text-xs text-red-600/80">Reference: {requestId}</p>}
    </div>
  );
}
...
function buildInstallConfig(data) {
  ...
  return {
    ...
    services: {
      ai_booking_agent: { booking_link: bookingLink, booking_mode: "external_link" },
      review_request: { review_link: googleReviewLink },
    },
    integrations: {
...
export default function CredentialsWizardHardened({
  order,
  setupToken = "",
  initialSection = "",
  ...
  const serverDraftRecord = order?.credentials_draft || null;
  const serverDraft = serverDraftRecord?.data || null;
  const serverDraftStep = serverDraftRecord?.current_step;
  const requestedTarget = useMemo(() => SECTION_TARGETS[initialSection] || null, [initialSection]);
  const requestedStep = requestedTarget?.step;

  const initialData = useMemo(() => {
    const defaults = defaultsForOrder(order);
    try {
      const saved = sessionStorage.getItem(storageKey);
      if (saved) return { ...defaults, ...(serverDraft || {}), ...JSON.parse(saved) };
    } catch {}
    return { ...defaults, ...(serverDraft || {}) };
  }, [order, serverDraft, storageKey]);

  const [authorized, setAuthorized] = useState(false);
  const [currentStep, setCurrentStep] = useState(() => requestedStep ?? clampStep(serverDraftStep, 0));
  ...
  const [draftStatus, setDraftStatus] = useState(
    serverDraftRecord?.updated_at
      ? `Draft restored from ${new Date(serverDraftRecord.updated_at).toLocaleString()}`
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const result = await base44.functions.invoke("checkSetupAuthorization", {
          order_id: order?.id,
        ...
        if (payload?.authorized) {
          setAuthorized(true);
          setCurrentStep((step) => requestedStep ?? Math.max(1, clampStep(serverDraftStep ?? step, 1)));
        }
      } catch {}
    };
    if (order?.id) checkAuth();
  }, [order?.id, serverDraftStep, effectiveSetupToken, requestedStep]);
  ...
  useEffect(() => {
    if (!authorized || !order?.id || currentStep === 0 || saving) return;
    const timer = setTimeout(async () => {
      try {
        const result = await base44.functions.invoke("saveClientCredentialsDraft", {
          order_id: order.id,
          token: effectiveSetupToken,
          draft: data,
        ...
        const payload = readPayload(result);
        setDraftStatus(`Draft saved ${new Date(payload.saved_at || Date.now()).toLocaleTimeString()}`);
      } catch (err) {
        const parsed = parseSaveError(err);
        setDraftStatus(`Draft not saved${parsed.requestId ? ` · ${parsed.requestId}` : ""}`);
      }
    }, 900);
    return () => clearTimeout(timer);
  }, [authorized, currentStep, data, order?.id, saving, effectiveSetupToken]);

  useEffect(() => {
    if (!authorized || !requestedTarget?.fieldId || currentStep !== requestedTarget.step) return;
    const timer = setTimeout(() => {
      document.getElementById(requestedTarget.fieldId)?.scrollIntoView({ behavior: "smooth", block: "center" });
    }, 120);
    return () => clearTimeout(timer);
  }, [authorized, currentStep, requestedTarget]);
  ...
  const handleLogoUpload = async (event) => {
    ...
    try {
      ...
      setError("Logo upload failed. Please try a smaller PNG or JPG.");
    } finally {
  ...
  const validateStep = () => {
    if (STEPS[currentStep].id === "business") {
      if (!trim(data.business_name)) return { field: "business_name", message: "Business name is required." };
      if (!trim(data.industry)) return { field: "industry", message: "Industry / niche is required." };
      if (!trim(data.business_phone)) return { field: "business_phone", message: "Business phone is required." };
      if (!isValidUSPhone(data.business_phone)) return { field: "business_phone", message: "Enter a valid US business phone number." };
      if (trim(data.business_email) && !isValidEmail(data.business_email)) return { field: "business_email", message: "Enter a valid business email address." };
      if (!isValidUrl(data.website)) return { field: "website", message: "Enter a valid website URL or leave it blank." };
    }
    if (STEPS[currentStep].id === "messaging") {
      if (trim(data.twilio_business_phone) && !isValidUSPhone(data.twilio_business_phone)) {
        return { field: "twilio_business_phone", message: "Enter a valid Twilio/business SMS phone number or leave it blank." };
      }
      if (!trim(data.booking_link)) return { field: "booking_link", message: "Booking link is required." };
      if (!isValidUrl(data.booking_link)) return { field: "booking_link", message: "Enter a valid booking link." };
      if (!trim(data.lead_notification_email)) return { field: "lead_notification_email", message: "Lead notification email is required." };
      if (!isValidEmail(data.lead_notification_email)) return { field: "lead_notification_email", message: "Enter a valid lead notification email." };
    }
    if (STEPS[currentStep].id === "integrations" && !isValidUrl(data.google_review_link)) {
      return { field: "google_review_link", message: "Enter a valid Google review link or leave it blank." };
    }
    return null;
  };
  ...
  const submit = async () => {
    ...
    try {
      const result = await base44.functions.invoke("saveClientCredentials", {
        order_id: order.id,
        token: effectiveSetupToken,
        install_configuration: buildInstallConfig(data),
      });
      const payload = readPayload(result);
      if (payload?.success === false || payload?.error) throw payload;
      try {
  ...
  const handleNext = async () => {
    ...
    if (currentStep === STEPS.length - 1) await submit();
    else setCurrentStep((step) => Math.min(step + 1, STEPS.length - 1));
  };
        ...
        <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-8">
          <div className="bg-white rounded-2xl border border-border p-6 shadow-sm">
            <SetupAuthorizationStep
              ...
              onAuthorized={() => {
                setAuthorized(true);
                setCurrentStep((step) => requestedStep ?? Math.max(1, step));
              }}
            />
          </div>
        </div>
  ...
  const completedSections = Math.max(0, currentStep - 1);
  const totalSetupSections = STEPS.length - 1;
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-8">
        <div className="mb-5 rounded-2xl border border-sky-100 bg-white p-4 shadow-sm flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.14em] text-primary">Setup checklist</p>
            <p className="mt-1 text-sm font-semibold text-foreground">{completedSections} of {totalSetupSections} setup sections completed</p>
            <p className="mt-1 text-xs text-muted-foreground">{draftStatus}</p>
          </div>
          <button
            type="button"
            onClick={handleSaveAndExit}
            disabled={saving || logoUploading}
            className="inline-flex items-center justify-center gap-2 rounded-xl border border-border bg-white px-4 py-2.5 text-sm font-semibold text-foreground hover:bg-muted transition-colors disabled:opacity-50"
          >
        ...
        {requestedTarget && currentStep === requestedTarget.step && (
          <div className="mb-5 rounded-2xl border border-sky-200 bg-sky-50 px-4 py-3">
            <p className="text-[10px] font-black uppercase tracking-[0.14em] text-sky-700">Opened from your dashboard</p>
            <p className="mt-1 text-sm font-bold text-slate-900">{requestedTarget.title}</p>
            <p className="mt-1 text-xs leading-5 text-slate-600">{requestedTarget.description}</p>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-border p-6 shadow-sm">
          {step.id === "business" && (
            <BusinessStep data={data} onChange={onChange} errors={fieldErrors} targetField={requestedTarget?.fieldId} />
          )}
          {step.id === "brand" && (
            <BrandStep data={data} onChange={onChange} onLogoUpload={handleLogoUpload} logoUploading={logoUploading} />
          )}
          {step.id === "messaging" && (
            <MessagingStep data={data} onChange={onChange} errors={fieldErrors} targetField={requestedTarget?.fieldId} />
          )}
          {step.id === "integrations" && <IntegrationsStep data={data} onChange={onChange} errors={fieldErrors} />}
          {step.id === "connection" && <IntegrationStatusStep data={data} />}
          {step.id === "review" && <ReviewStep data={data} />}
          <ErrorPanel error={error} fieldErrors={fieldErrors} requestId={requestId} />
        </div>
