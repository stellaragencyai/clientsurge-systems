...
function parseSaveError(err) {
  ...
  return { message, fieldErrors, requestId: data.request_id || err?.request_id || "" };
}
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="Booking / Scheduling Link" required error={errors.booking_link}>
...
export default function CredentialsWizardHardened({ order, onComplete }) {
    ...
    const checkAuth = async () => {
      try {
        ...
        if (payload?.authorized) {
          setAuthorized(true);
          setCurrentStep((step) => Math.max(1, Number(serverDraftStep) || step || 1));
        }
      } catch {}
    };
    if (order?.id) checkAuth();
  }, [order?.id, serverDraftStep]);
  ...
  }, [authorized, currentStep, data, order?.id, saving]);
  ...
  const validateStep = () => {
    ...
    if (STEPS[currentStep].id === "messaging") {
      if (trim(data.twilio_business_phone) && !isValidUSPhone(data.twilio_business_phone)) return { field: "twilio_business_phone", message: "Enter a valid Twilio/business SMS phone number or leave it blank." };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-8">
        ...
        <div className="bg-white rounded-2xl border border-border p-6 shadow-sm">
          {step.id === "business" && <BusinessStep data={data} onChange={onChange} errors={fieldErrors} />}
          {step.id === "brand" && <BrandStep data={data} onChange={onChange} onLogoUpload={handleLogoUpload} logoUploading={logoUploading} />}
          {step.id === "messaging" && <MessagingStep data={data} onChange={onChange} errors={fieldErrors} />}
        ...
        <div className="flex items-center justify-between gap-4 mt-6">
          <button
            onClick={() => { setError(""); setFieldErrors({}); setCurrentStep((step) => Math.max(1, step - 1)); }}
          ...
          <div className="flex flex-col items-end gap-2">
            {draftStatus && (
              <p className="flex items-center gap-1 text-xs text-muted-foreground">
                <CheckCircle2 className="w-3.5 h-3.5" /> {draftStatus}
              </p>
            )}
            <button
              onClick={handleNext}
