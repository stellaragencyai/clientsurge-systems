...
export default function LeadBulkToolbar({ selectedIds, onClear, onDone }) {
  const [action, setAction] = useState(null); // 'status' | 'sequence' | 'delete'
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await Promise.all(selectedIds.map(id => base44.entities.Leads.delete(id)));
      onDone("delete");
    } finally {
      setLoading(false);
      setAction(null);
    ...
    >
      ...
      {action === "delete" && (
        <div className="flex items-center gap-2 flex-1">
          {confirm ? (
            <>
              <span className="text-xs text-red-400 font-semibold">Delete {count} leads?</span>
              <button
                onClick={handleDelete}
                disabled={loading}
                className="text-xs font-bold px-3 py-1.5 rounded-lg bg-red-500 text-white disabled:opacity-50"
              >
                {loading ? <Loader2 className="w-3 h-3 animate-spin" /> : "Confirm"}
              </button>
              <button onClick={() => { setAction(null); setConfirm(false); }} className="text-white/60 hover:text-white">
                <X className="w-4 h-4" />
              </button>
            </>
          ) : null}
      ...
      {!action && (
        <div className="flex items-center gap-1.5 flex-1">
          ...
          <button
            onClick={() => { setAction("delete"); setConfirm(false); handleDelete(); }}
            className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-red-500/80 hover:bg-red-500 transition-colors"
          >
