...
export default function SupportChat({ project, user }) {
  ...
  const loadMessages = async () => {
    ...
    msgs.filter(m => m.role === "admin" && !m.read).forEach(m =>
  ...
  return (
    <div className="bg-white rounded-2xl border border-border shadow-sm flex flex-col" style={{ height: "460px" }}>
      ...
      <div className="px-6 py-4 border-b border-border">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">Message the ApexFlow team directly. We reply within a few hours.</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
        ...
        {!loading && messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Send us a message below — we typically reply within a few hours.</p>
          </div>
        )}
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === "client" ? "justify-end" : "justify-start"}`}>
            ...
            >
              {msg.role === "admin" && (
                <p className="text-xs font-bold mb-1 opacity-60">ApexFlow Team</p>
              )}
              {msg.message}
            </div>
          </div>
