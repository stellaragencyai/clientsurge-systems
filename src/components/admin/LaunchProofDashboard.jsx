...
import LaunchProofDeploymentReleaseSection from "./LaunchProofDeploymentReleaseSection";
...
export default function LaunchProofDashboard() {
  ...
  const getCardStatus = (cardKey) => {
    ...
    switch (cardKey) {
      case "stripe": {
        ...
        const missingPaidOrderOnly = gate?.status === "blocked" && String(gate?.current_blocker || gate?.evidence_summary || "").toLowerCase().includes("no real non-test paid order");
        const status = hasProductionPaidOrder ? "trusted" : missingPaidOrderOnly ? "ready" : gateStatusToCardStatus(gate);
        return { status, nextAction: status === "trusted" ? "" : gate?.next_action || "Complete a real Stripe checkout. Verify payment_status=paid on a production order." };
      }
      case "analytics": {
        ...
        const hasRealConversionEvents = Boolean(ga4.has_real_conversion_events || ga4.has_tracking_proof || (pageViewCount > 0 && ctaClickCount > 0));
        const ga4Active = Boolean(ga4.ga4_active || ga4.status === "ready_for_proof" || ga4.status === "trusted" || (ga4.record_exists && ga4.measurement_id_valid && ga4.tracking_enabled && ga4.setup_status === "active"));
        if (ga4Active && hasRealConversionEvents) return { status: "trusted", nextAction: "" };
        if (ga4Active) return { status: "ready", nextAction: ga4.next_action || "GA4 is configured, but no real page_view + cta_click proof has been recorded yet. Generate traffic and verify in GA4 Realtime." };
        return { status: "blocked", nextAction: ga4.next_action || "Configure GA4 with a valid measurement ID and enable tracking." };
      }
      case "dashboard_truth": {
        ...
        const unresolvedProductionIssues = Number(dashboardTruth.failed_jobs_production || 0) + Number(dashboardTruth.stuck_jobs_production || 0) + Number(dashboardTruth.dead_letter_production || 0);
        if (gate?.status === "approved" || gate?.status === "proof_passed") return { status: "trusted", nextAction: "" };
        if (gate?.status === "ready_for_proof" || (dashboardTruth.safe_to_show_admin === true && dashboardTruth.safe_to_launch === true && unresolvedProductionIssues === 0)) {
          return { status: "ready", nextAction: gate?.next_action || "Admin approval required after reviewing dashboard truth evidence." };
        }
        if (gate?.status === "blocked" || unresolvedProductionIssues > 0) {
          return { status: "blocked", nextAction: gate?.next_action || "Resolve production-trusted failed/stuck jobs and dead-letter records before launch." };
        }
        if (leadCapture?.latest_website_lead?.is_production_trusted || dashboardTruth.safe_to_show_admin === true || leadCapture?.latest_website_lead) {
          return { status: "warning", nextAction: gate?.next_action || "Dashboard truth evidence exists, but it is not ready for proof yet." };
        }
      ...
      case "voice": {
        ...
        return { status, nextAction: status === "trusted" ? "" : gate?.next_action || "Configure ElevenLabs/Twilio voice, make a real inbound test call, then rerun proof." };
      }
      case "booking": {
        ...
        if (gateStatus === "ready" || (linkPresent && linkValid)) return { status: "ready", nextAction: gate?.next_action || booking.next_action || "Open the booking link, verify the calendar loads, complete a test booking/click, then approve proof." };
        if (linkPresent) return { status: "warning", nextAction: gate?.next_action || booking.next_action || "Booking link is set but appears invalid. Update in Admin Settings." };
        return { status: "blocked", nextAction: gate?.next_action || booking.next_action || "Set a valid booking link (Calendly or similar) in Admin Settings." };
      }
      case "site_readiness": {
  ...
  const sortedCards = [...PROOF_CARDS].sort((a, b) => (statusOrder[getCardStatus(a.key).status] ?? 5) - (statusOrder[getCardStatus(b.key).status] ?? 5));
  const blockedCount = PROOF_CARDS.filter((card) => getCardStatus(card.key).status === "blocked").length;
  const warningCount = PROOF_CARDS.filter((card) => getCardStatus(card.key).status === "warning").length;
  const readyCount = PROOF_CARDS.filter((card) => getCardStatus(card.key).status === "ready").length;
  const trustedCount = PROOF_CARDS.filter((card) => getCardStatus(card.key).status === "trusted").length;
  const deploymentEvidence = data?.evidence?.deployment_release || data?.sections?.deployment_release || data?.deployment_release || {};
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <button onClick={fetchProof} disabled={loading} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-border text-sm font-semibold text-foreground hover:bg-muted transition-colors disabled:opacity-60">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-center"><p className="text-2xl font-bold text-red-700">{blockedCount}</p><p className="text-xs font-semibold uppercase text-red-700 tracking-wide">Blocked</p></div>
        <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-3 text-center"><p className="text-2xl font-bold text-yellow-700">{warningCount}</p><p className="text-xs font-semibold uppercase text-yellow-700 tracking-wide">Warning</p></div>
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-center"><p className="text-2xl font-bold text-blue-700">{readyCount}</p><p className="text-xs font-semibold uppercase text-blue-700 tracking-wide">Ready</p></div>
        <div className="rounded-lg border border-green-200 bg-green-50 p-3 text-center"><p className="text-2xl font-bold text-green-700">{trustedCount}</p><p className="text-xs font-semibold uppercase text-green-700 tracking-wide">Trusted</p></div>
      </div>
      ...
      <LaunchProofDeploymentReleaseSection evidence={deploymentEvidence} />
