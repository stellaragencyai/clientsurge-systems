...
import { buildAutomationProof } from '@/lib/automationProof';
...
const STATUS = {
  ...
  failing: { label: 'Needs Review', tone: 'border-red-200 bg-red-50 text-red-800', Icon: AlertTriangle },
  ...
  return (
    <div className={`rounded-xl border p-4 ${config.tone}`}>
      ...
      <div className="mt-4 grid grid-cols-4 gap-2 text-center text-xs">
        ...
        <div className="rounded-lg bg-white/70 p-2"><p className="font-bold text-green-700">{item.sent_count}</p><p className="opacity-70">Sent</p></div>
        <div className="rounded-lg bg-white/70 p-2"><p className="font-bold text-blue-700">{item.skipped_count}</p><p className="opacity-70">Guarded</p></div>
        <div className="rounded-lg bg-white/70 p-2"><p className="font-bold text-red-700">{item.failed_count}</p><p className="opacity-70">Issues</p></div>
      </div>
...
export default function AutomationEvidenceCards() {
  ...
  const proof = useMemo(() => buildAutomationProof(events), [events]);
  const proven = proof.filter((item) => item.status === 'proven').length;
  const guarded = proof.filter((item) => item.status === 'guarded').length;
  const review = proof.filter((item) => item.status === 'failing' || item.status === 'mixed').length;
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Evidence from recent CommunicationEvent records. These cards show what ran, what sent, what was guarded, and what needs review.</p>
        </div>
