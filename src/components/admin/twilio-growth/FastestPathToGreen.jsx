...
export default function FastestPathToGreen({ data }) {
  ...
  const items = caps.map(cap => {
    ...
    const nextBlockerToRemove = cap.blockers?.[0] || (hasBlockers ? "Resolve active blockers listed in the capability matrix." : "No active blockers — maintain current state.");
          ...
          <div key={item.key} className="bg-white rounded-xl border border-gray-200 p-5" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.03)" }}>
            ...
            <div className="space-y-2.5">
              ...
              <div>
                ...
                <p className="text-xs text-gray-600 leading-relaxed">{item.nextBlockerToRemove}</p>
              </div>
