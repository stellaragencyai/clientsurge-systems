...
export default function AdminWebsiteLeadsTab() {
  ...
  const displayed = leads.filter(l => {
    ...
    const matchSearch = !search || [l.full_name, l.email, l.phone, l.business_name]
          ...
          {displayed.map(lead => (
            <div key={lead.id} style={{
              ...
              <div style={{ flex: 1, minWidth: 200 }}>
                ...
                <p style={{ color: "#9CA3AF", fontSize: 12, margin: 0 }}>{lead.email} · {lead.phone}</p>
              </div>
              <p style={{ color: "#6B7280", fontSize: 12, margin: 0, minWidth: 120 }}>{lead.business_name || "—"}</p>
              <p style={{ color: "#6B7280", fontSize: 12, margin: 0, minWidth: 80 }}>{lead.industry || "—"}</p>
              <span style={{
                background: lead.status === "booked" ? "rgba(0,255,179,0.1)" : "rgba(255,255,255,0.05)",
                color: lead.status === "booked" ? "#00FFB3" : "#9CA3AF",
