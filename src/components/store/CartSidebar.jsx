...
export default function CartSidebar() {
  const { items, removeItem, cartOpen, setCartOpen, totalSetup, totalMonthly, clearCart } = useCart();
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 24px", borderBottom: "1px solid rgba(154,92,46,0.12)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            {items.length > 0 && <span style={{ background: "#9a5c2e", color: "#fff", borderRadius: "9999px", width: "20px", height: "20px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "11px", fontWeight: "700" }}>{items.length}</span>}
          </div>
          <button onClick={() => setCartOpen(false)} style={{ background: "none", border: "none", cursor: "pointer", padding: "4px" }}>
          ...
          {items.length === 0 ? (
              ...
              {items.map((item) => (
                <div key={item.product_id} style={{ background: "rgba(255,255,255,0.8)", border: "1px solid rgba(154,92,46,0.12)", borderRadius: "14px", padding: "14px 16px", display: "flex", alignItems: "center", gap: "12px" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: "13px", fontWeight: "700", color: "#1a1209", margin: "0 0 3px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item.name}</p>
                    <p style={{ fontSize: "11px", color: "rgba(26,18,9,0.5)", margin: 0 }}>${item.setup_fee} setup · ${item.monthly_fee}/mo</p>
                  </div>
                  <button onClick={() => removeItem(item.product_id)} style={{ background: "none", border: "none", cursor: "pointer", padding: "4px", color: "#e57373" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
              ...
              {[
                ...
                <div key={f.key}>
                  <label style={{ fontSize: "11px", fontWeight: "700", color: "rgba(26,18,9,0.55)", textTransform: "uppercase", letterSpacing: "0.08em", display: "block", marginBottom: "5px" }}>{f.label}</label>
                  <input
          ...
          <div style={{ padding: "16px 24px", borderTop: "1px solid rgba(154,92,46,0.12)", background: "rgba(255,255,255,0.6)", backdropFilter: "blur(10px)" }}>
            ...
            {(step === "info" || step === "loading") && (
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                ...
                >
                  <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", height: "48px", borderRadius: "9999px", background: "linear-gradient(135deg,#6b3f1f 0%,#9a5c2e 40%,#7a4825 100%)", color: "#f5e6d0", fontWeight: "700", fontSize: "14px" }}>
                    {step === "loading" ? "Redirecting to Stripe..." : <><Lock style={{ width: "13px", height: "13px" }} /> Pay Securely with Stripe</>}
                  </span>
                </button>
                <button onClick={() => setStep("cart")} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "12px", color: "rgba(26,18,9,0.4)", textDecoration: "underline" }}>
                  ← Back to cart
                </button>
              </div>
            )}
