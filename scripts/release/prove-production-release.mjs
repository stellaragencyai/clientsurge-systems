...
const STAGING_APP_IDS = [
  "69f959e2bc665e019e19840c",
  "6a15f1424f4856ba4e9ed90b",
];
...
function evaluateRoute(result) {
  ...
  if (result.statusCode < 200 || result.statusCode >= 400) {
    failures.push(`HTTP ${result.statusCode}`);
  }
...
function buildMarkdown(report) {
  ...
  lines.push("");
  lines.push(`## Manual evidence required before calling the release live`);
  lines.push("");
  for (const item of report.manual_evidence_required) {
    lines.push(`- [ ] ${item}`);
  }
  lines.push("");
  return `${lines.join("\n")}\n`;
