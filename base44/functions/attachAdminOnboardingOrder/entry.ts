...
Deno.serve(async (req) => {
  try {
    ...
    if (!onboarding_client_id || !order_id) {
      return secureJson({ error: "onboarding_client_id and order_id are required" }, { status: 400 });
    }
    ...
    if (!client) return secureJson({ error: "Onboarding client not found" }, { status: 404 });
    ...
    return secureJson({ success: true, onboarding_client: updatedClient, order_id });
  } catch (error) {
