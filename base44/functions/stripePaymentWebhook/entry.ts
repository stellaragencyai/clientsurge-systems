 ...
 * both live and test webhook signing secrets during launch proofing.
 */
function normalizePackageKey(value) {
  const normalized = String(value || "").trim().toLowerCase();
  const aliases = {
    starter: "basic_website_plus_two_automations",
    basic: "basic_website_plus_two_automations",
    growth: "growth_website_plus_four_automations",
    pro: "pro_website_plus_six_automations",
    elite: "pro_website_plus_six_automations",
  };
...
async function initializePaidOrderInstallPipeline({ base44, order }) {
  ...
  if (!data.success) {
    throw new Error(data.error || "Install pipeline initialization failed");
  }
  return {
    order: data.order || order,
    client: data.client || null,
    clientProject: data.project || null,
    onboardingClient: data.onboarding_client || null,
    createdEvents: [],
  };
}
...
async function resolveLeadForWonOrder(base44, order) {
  ...
  if (customerEmail) {
    ...
    if (exact || matches?.[0]) return exact || matches[0];
  }
