...
function adminSubjectLabelForLead(lead, industryTags = '') {
  const combined = `${lead.business_type || ''} ${lead.industry || ''} ${lead.crm_tag || ''} ${industryTags}`.toLowerCase();
  if (combined.includes('roofing') || combined.includes('roof')) return 'New Roofing Lead';
  if (combined.includes('hvac')) return 'New HVAC Lead';
  if (combined.includes('dental') || combined.includes('orthodont')) return 'New Dental Lead';
  if (combined.includes('med_spa') || combined.includes('med-spa') || combined.includes('aesthetic')) return 'New Med Spa Lead';
  if (combined.includes('plumb')) return 'New Plumbing Lead';
  return 'New Lead';
}
...
Deno.serve(async (req) => {
  try {
    ...
    const industryTags = Array.isArray(lead.industry_tags)
      ...
      : '';
    const adminSubjectLabel = adminSubjectLabelForLead(lead, industryTags);
  ...
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;border:1px solid #e5e5e5;overflow:hidden;">
    ...
    const res = await resendFetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `${adminSubjectLabel}: ${lead.full_name} - ${lead.business_name || lead.business_type || 'Unknown'}`,
        html: htmlBody,
      }),
    });
