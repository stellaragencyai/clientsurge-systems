...
/**
 * sendOrderConfirmationEmail — #404 #404a #404c
 * FIX: Removed broken local file imports (salesCatalog.js, serviceRows.shared.js)
 * that attempted to import frontend source code into Deno runtime — always failed.
 * FIX: Removed TypeScript type annotations from .js file.
 * FIX: Inlined all helper logic.
 */

// Inlined package catalog (canonical pricing)
...
function resolveServiceRows(order, packageOffer) {
  // Prefer explicit items array
  if (Array.isArray(order.items) && order.items.length > 0) {
    return order.items.map(item => ({
      name: item.product_name || item.service_key || "Service",
      setup_fee: item.setup_fee || 0,
      monthly_fee: item.monthly_fee || 0,
    }));
  }
  // Fallback to package-level totals
  if (packageOffer) {
    return [{
      name: packageOffer.name,
      setup_fee: order.total_setup || packageOffer.setup_total,
      monthly_fee: order.total_monthly || packageOffer.monthly_total,
    }];
  }
  return [];
}
...
// #404a: Tier-aware email copy per package
const TIER_MESSAGING = {
  starter_system: {
    activation_copy: "Your 2 AI automation systems are activating — Instant Lead Response and Missed Call Text-Back.",
    timeline: "Systems are typically live within 48–72 hours.",
  },
  growth_system: {
    activation_copy: "Your 4 AI automation systems are activating — Lead Response, Missed Call Text-Back, 14-Day Nurture, and AI Booking Agent.",
    timeline: "Systems are typically live within 48–72 hours.",
  },
  pro_system: {
    activation_copy: "Your complete AI automation stack is activating — all 6 systems including Lead Response, Missed Call, Nurture, Booking Agent, Lead Reactivation, and Review Requests.",
    timeline: "Your Pro setup includes a custom AI-built website. Full system goes live within 5–7 business days.",
  },
};
...
Deno.serve(async (req) => {
  try {
    ...
    if (!customerEmail) return Response.json({ error: "Order missing customer_email" }, { status: 400 });
    ...
    const portalUrl = portal_activation_url || `${getAppUrl()}/client-portal`;
    const credentialsUrl = `${getAppUrl()}/setup/credentials?order_id=${order.id}`;
    ...
    const serviceListHtml = serviceRows.map(svc =>
      `<li style="margin-bottom:8px;color:#374151;"><strong>${escapeHtml(svc.name)}</strong>${svc.setup_fee > 0 || svc.monthly_fee > 0 ? ` — $${formatMoney(svc.setup_fee)} setup / $${formatMoney(svc.monthly_fee)}/mo` : ""}</li>`
    ).join("");

    const serviceListText = serviceRows.map(svc =>
      `- ${svc.name}${svc.setup_fee > 0 || svc.monthly_fee > 0 ? `: $${formatMoney(svc.setup_fee)} setup / $${formatMoney(svc.monthly_fee)}/mo` : ""}`
    ).join("\n");
    ...
    const text = [
      ...
      order.total_setup ? `Setup: $${formatMoney(order.total_setup)}` : "",
      order.total_monthly ? `Monthly: $${formatMoney(order.total_monthly)}/mo` : "",
      "",
      "Included services:",
      serviceListText || "- Service bundle",
      "",
      "What happens next:",
      tierMsg.activation_copy,
      tierMsg.timeline,
      "",
      `Complete Setup: ${credentialsUrl}`,
    ...
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        reply_to: replyTo,
        to: customerEmail,
        subject: `Order confirmed — ${packageLabel} | ClientSurge`,
        html,
        text,
      }),
    });
    ...
    if (!response.ok) {
      ...
      console.error("[sendOrderConfirmationEmail] Resend failed:", errText);
      throw new Error(`Resend error: ${response.status} ${errText}`);
    }

    console.log("[sendOrderConfirmationEmail] Sent to:", customerEmail, "Package:", packageLabel);
