...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const supportEmail = Deno.env.get("CLIENTSURGE_SUPPORT_EMAIL") || Deno.env.get("SUPPORT_EMAIL") || "support@clientsurgesystems.com";
    ...
    const pkgKey = resolvePackageKey(order.pricing_summary?.package_key || order.package_key || order.package_type || order.selected_package_type);
    const packageOffer = getPackageOffer(pkgKey);
    const packageLabel = packageOffer?.name || order.plan_type || "Custom Service Bundle";
