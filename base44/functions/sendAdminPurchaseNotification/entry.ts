 ...
 * Sends a purchase alert email to the admin when a customer completes checkout.
 */
...
Deno.serve(async (req) => {
  try {
    ...
    if (!toEmail) {
      return Response.json({ skipped: true, reason: "No admin notification email configured" });
    }
    ...
    const customerName = escapeHtml(body.customer_name || "Unknown");
    const customerEmail = escapeHtml(body.customer_email || "—");
    const customerPhone = escapeHtml(body.customer_phone || "—");
    const businessName = escapeHtml(body.business_name || "Unknown");
    ...
    const itemList = items
      .map(
        (item) =>
          `<li style="padding:4px 0;font-size:13px;color:#1a1209;">${escapeHtml(item.icon || "")} <strong>${escapeHtml(item.product_name)}</strong> — ${item.setup_fee > 0 ? `$${item.setup_fee} setup + ` : ""}$${item.monthly_fee}/mo</li>`
      )
      .join("");
  ...
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;border:1px solid #e5e5e5;overflow:hidden;">
    ...
    <div style="padding:28px;">
      <table style="width:100%;border-collapse:collapse;font-size:14px;margin-bottom:20px;">
        <tr><td style="padding:8px 0;color:#888;width:130px;font-weight:600;">Customer</td><td style="padding:8px 0;color:#1a1a1a;font-weight:700;">${customerName}</td></tr>
        <tr style="background:#fafafa;"><td style="padding:8px 6px;color:#888;font-weight:600;">Business</td><td style="padding:8px 6px;color:#1a1a1a;">${businessName}</td></tr>
        <tr><td style="padding:8px 0;color:#888;font-weight:600;">Email</td><td style="padding:8px 0;color:#1a1a1a;">${customerEmail}</td></tr>
        <tr style="background:#fafafa;"><td style="padding:8px 6px;color:#888;font-weight:600;">Phone</td><td style="padding:8px 6px;color:#1a1a1a;">${customerPhone}</td></tr>
      ...
      <div style="background:#f0fdf4;border:1px solid rgba(34,197,94,0.25);border-radius:10px;padding:16px 18px;margin-bottom:20px;">
        ...
        <div style="margin-top:14px;padding-top:12px;border-top:1px solid rgba(34,197,94,0.15);">
          <div style="font-size:13px;color:rgba(26,18,9,0.6);">One-time setup: <strong>${totalSetup > 0 ? `$${totalSetup}` : "No setup fee"}</strong></div>
          <div style="font-size:15px;font-weight:800;color:#15803d;margin-top:2px;">Monthly recurring: $${totalMonthly}/mo</div>
        </div>
      </div>

      <a href="${appUrl}/admin" style="display:inline-block;background:linear-gradient(135deg,#6b3f1f,#9a5c2e);color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:700;font-size:14px;">
    ...
    const textBody = [
      ...
      "",
      "New Purchase",
      `Customer: ${cleanString(body.customer_name) || "Unknown"}`,
      `Business: ${cleanString(body.business_name) || "Unknown"}`,
      `Email: ${cleanString(body.customer_email) || "—"}`,
      `Phone: ${cleanString(body.customer_phone) || "—"}`,
      `Purchased At: ${purchasedAt} MST`,
      "",
      "Services:",
      ...(items.length > 0
        ? items.map(
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: toEmail,
        subject: `💰 New Purchase — ${businessName} ($${totalMonthly}/mo)`,
        html: htmlBody,
        text: textBody,
      }),
    });
    ...
    if (orderId) {
      await base44.asServiceRole.entities.CommunicationEvent.create({
        ...
        provider: "resend",
        status: "sent",
        subject: `Purchase notification sent to ${toEmail}`,
        message_body: "Admin purchase notification email sent successfully.",
        provider_message_id: resendData?.id,
        metadata_json: JSON.stringify({
          target: "admin_purchase_notification",
          to_email: toEmail,
        }),
      });
    }
