...
export function renderMasterFallbackHtml(input: {
  ...
  const cta = input.ctaLabel && input.ctaUrl
    ? `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:26px 0 4px 0;"><tr><td bgcolor="#126bff" style="border-radius:12px;"><a href="${escapeHtml(input.ctaUrl)}" style="display:inline-block;padding:15px 22px;color:#ffffff;text-decoration:none;font-size:15px;line-height:20px;font-weight:800;border-radius:12px;">${escapeHtml(input.ctaLabel)}</a></td></tr></table>`
    : "";
...
<html lang="en">
  ...
  <body style="margin:0;padding:0;background:#f4f8ff;color:#0b1749;font-family:Arial,Helvetica,sans-serif;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse;background:#f4f8ff;"><tr><td align="center" style="padding:28px 12px;">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:680px;width:100%;border-collapse:separate;border-spacing:0;background:#ffffff;border:1px solid #dce8ff;border-radius:24px;overflow:hidden;">
        <tr><td style="padding:28px 30px 18px 30px;border-bottom:1px solid #e8eefb;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td><div style="font-size:23px;line-height:29px;font-weight:900;letter-spacing:-0.7px;color:#06164b;">ClientSurge <span style="color:#126bff;">Systems</span></div></td><td align="right"><span style="display:inline-block;background:#eaf2ff;color:#126bff;border:1px solid #c9ddff;border-radius:999px;padding:8px 12px;font-size:12px;line-height:14px;font-weight:900;text-transform:uppercase;letter-spacing:0.4px;">${escapeHtml(input.badge)}</span></td></tr></table></td></tr>
        <tr><td style="padding:30px;">
          <h1 style="margin:0 0 12px 0;color:#06164b;font-size:28px;line-height:34px;font-weight:900;letter-spacing:-0.8px;">${escapeHtml(input.headline)}</h1>
          <p style="margin:0;color:#344054;font-size:16px;line-height:25px;">${escapeHtml(input.intro)}</p>
          ${cta}
          ${rows.length ? `<div style="background:#f8fbff;border:1px solid #dce8ff;border-radius:18px;padding:20px;margin-top:22px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">${rows.map(([label, value]) => `<tr><td style="padding:9px 0;border-bottom:1px solid #e8eefb;color:#667085;font-size:13px;line-height:19px;width:40%;">${escapeHtml(label)}</td><td style="padding:9px 0;border-bottom:1px solid #e8eefb;color:#0b1749;font-size:14px;line-height:20px;font-weight:800;">${escapeHtml(value ?? "Not provided yet")}</td></tr>`).join("")}</table></div>` : ""}
          ${bullets.length ? `<div style="background:#ffffff;border:1px solid #dce8ff;border-radius:18px;padding:21px;margin-top:18px;"><h2 style="margin:0 0 10px 0;color:#06164b;font-size:18px;line-height:24px;font-weight:900;">What happens next</h2><ul style="margin:13px 0 0 0;padding:0;list-style:none;">${bullets.map((bullet) => `<li style="margin:0 0 9px 0;color:#0b1749;font-size:14px;line-height:21px;"><span style="display:inline-block;width:18px;height:18px;border-radius:999px;background:#eaf2ff;color:#126bff;text-align:center;font-size:12px;font-weight:900;line-height:18px;margin-right:8px;">✓</span>${escapeHtml(bullet)}</li>`).join("")}</ul></div>` : ""}
          <div style="background:#fff8e6;border:1px solid #ffe2a8;border-radius:16px;padding:16px 18px;margin-top:20px;"><p style="margin:0;color:#684d00;font-size:14px;line-height:22px;"><strong>Security note:</strong> Never send passwords by email. Use secure access links or access instructions only.</p></div>
        </td></tr>
        <tr><td style="padding:0 30px 30px 30px;"><div style="background:#06164b;border-radius:20px;padding:22px;color:#ffffff;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td style="width:72px;vertical-align:top;padding-right:14px;"><div style="width:58px;height:58px;border-radius:999px;background:#eaf2ff;color:#126bff;font-size:18px;line-height:58px;font-weight:900;text-align:center;border:2px solid #dce8ff;">CS</div></td><td style="vertical-align:top;"><p style="margin:0;color:#ffffff;font-size:16px;line-height:22px;font-weight:900;">${escapeHtml(input.senderName || "ClientSurge Setup Team")}</p><p style="margin:2px 0 10px 0;color:#b9cbff;font-size:13px;line-height:19px;font-weight:700;">${escapeHtml(input.senderTitle || "Automation Installation Team")}</p><p style="margin:0;color:#d8e4ff;font-size:13px;line-height:21px;">${escapeHtml(input.proofLine || "Replies go to a monitored ClientSurge inbox.")}</p><p style="margin:12px 0 0 0;color:#ffffff;font-size:13px;line-height:21px;"><a href="mailto:${escapeHtml(DEFAULT_SUPPORT_EMAIL)}" style="color:#ffffff;text-decoration:underline;">${escapeHtml(DEFAULT_SUPPORT_EMAIL)}</a><span style="color:#7188d8;"> • </span>${escapeHtml(DEFAULT_SUPPORT_PHONE)}<span style="color:#7188d8;"> • </span><a href="${escapeHtml(APP_URL)}" style="color:#ffffff;text-decoration:underline;">clientsurgesystems.com</a></p>${input.referenceId ? `<p style="margin:12px 0 0 0;color:#8ea8ff;font-size:11px;line-height:16px;">Reference: ${escapeHtml(input.referenceId)}</p>` : ""}</td></tr></table></div></td></tr>
      </table>
    </td></tr></table>
  </body>
</html>`;
